...
oeApp.constant('ngSortableConfig', {
	onEnd : function() {
...
function($scope, $http, $ionicHistory, $timeout, $templateCache, EventBus, $state, $ionicModal, $ionicScrollDelegate) {
	...
	$scope.receiveNotification = function(data, cbtype) {
		...
		EventBus.fire({
			type : "notice.index.receiveNotification",
			data : sysConfigData.notification
		});
		//接收极光推送信息广播
	};
	...
	$scope.$on('$stateChangeStart', function(event, toState, toParams, fromState, fromParams) {
		...
		$scope.showFooter = toState.showFooter;
